    ...
    <div class="container">
      ...
      <div class="filters">
        <div class="filter-group">
          <label>事件类型：</label>
          <select v-model="filters.eventType" multiple @change="applyFilters">
      ...
      <table v-else class="event-table">
        ...
        <tbody>
          <tr v-for="event in events" :key="event.id">
            ...
            <td>{{ event.user_battletag || event.user_id }}</td>
            <td>{{ event.event_type }}</td>
            <td>{{ event.target_battletag || event.target_user_id || '-' }}</td>
...
async function fetchEvents() {
  ...
  try {
    ...
    if (filters.value.eventType.length) {
      filters.value.eventType.forEach(t => params.append('eventType', t));
    }
    if (filters.value.startTime) params.append('startTime', new Date(filters.value.startTime).toISOString());
    if (filters.value.endTime) params.append('endTime', new Date(filters.value.endTime).toISOString());
...
function resetFilters() {
  filters.value = { eventType: [], startTime: '', endTime: '' };
  offset.value = 0;
  fetchEvents();
}

function changePage(delta: number) {
  const newOffset = offset.value + delta * limit.value;
